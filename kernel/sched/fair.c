...
static int select_energy_cpu_brute(struct task_struct *p, int prev_cpu, int sibling_count_hint, int sync_boost)
{
		    ...
		    int sibling_count_hint)
{
	...
	if (energy_aware()) {
		...
		rcu_read_lock();
		new_cpu = select_energy_cpu_brute(p, prev_cpu, sibling_count_hint, sync_boost);
		rcu_read_unlock();
		return new_cpu;
	}
